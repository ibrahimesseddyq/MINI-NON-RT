...
/*   Updated: 2025/02/13 22:51:43 by sessarhi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void cal_uv_basis(const t_vector *normal, t_vector *u_axis, t_vector *v_axis)
{
    t_vector arbitrary;
    ...
    // Calculate u_axis as the cross product of normal and arbitrary
    *u_axis = vector_cross(normal, &arbitrary);
    *u_axis = vector_normalize(u_axis);

    // Calculate v_axis as the cross product of normal and u_axis
...
bool plane_intersection(const t_scene *scene, t_intersection *intersection, const t_ray *ray)
{
    ...
    while (pint.i--)
    {
        pint.t = hit_plane(&scene->plane[pint.i].position, &scene->plane[pint.i].direction, ray);
        if (pint.t > 0 && pint.t < intersection->distance)
        {
            ...
            pint.hpr = vector_sub(&intersection->point, &scene->plane[pint.i].position);
